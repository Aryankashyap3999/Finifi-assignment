...
**AI provider abstraction** (`backend/src/services/extraction/`): `documentService.js` calls `extractStructuredData(filePath, mimeType, prompt)` and never imports an AI SDK directly. `extraction/index.js` is the one place that picks a provider — by the `AI_PROVIDER` env var — from a plain object mapping a name to a function; each provider is just a file under `providers/` implementing that same `(filePath, mimeType, prompt) => Promise<object>` shape. Adding a new provider (Claude, OpenAI, ...) means writing one new file and adding one line to that map — no interfaces, classes, or DI container, since a plain function contract already gives the whole point of the abstraction (business logic depending on a shape, not an SDK) without any of the ceremony. `geminiProvider.js` (the default) and `groqProvider.js` (a working second provider, proving the abstraction isn't just theoretical) are both implemented; see [Known limitations](#known-limitations) for what's not fully solved by switching providers.
...
- **Gemini extraction is not perfectly deterministic, and this was not fixable with configuration or prompt tuning.** Re-parsing the *exact same* PO PDF repeatedly (to test `duplicate_po` detection) produced **five different `poNumber` values across six calls** — one correct (`CI4PO05788`), four fabricated (`PO-1819`, `PO-2023-001`, `PO-4091208`, `PO-2023-88492`), none of which appear anywhere in the source document's text (confirmed via `pdftotext`). Three separate fixes were tried and each ruled out in turn:
  1. `temperature: 0` (greedy decoding) — kept, since it's still correct practice, but a wrong value appeared on the very next call after adding it.
  2. A fixed `seed` alongside it — made things measurably *worse* (empty/unparseable output on two consecutive calls instead of a wrong-but-well-formed one) and was reverted.
  3. A strengthened prompt explicitly instructing the model to copy document numbers verbatim and never substitute a different number found elsewhere on the page (GSTIN, contact number, etc.) — still produced a fifth wrong value. Rendering the actual PDF page confirmed the field itself is completely unambiguous: `PO No : CI4PO05788` sits alone in a clean table cell, with no barcode, no nearby lookalike field, nothing that would explain a plausible visual confusion.

  Since duplicate detection keys on an exact `poNumber` match, this is a real, thoroughly-demonstrated way a genuine duplicate PO could be missed. This is a limitation of the underlying model's reliability on this specific field, not a bug anywhere in this codebase — the duplication logic itself was separately verified correct (with a genuine duplicate invoice, where the number stayed consistent), and the retry-then-fail-clearly path was separately verified working (a truly malformed response is retried once, then a clean 422). A config or prompt change was never going to be a complete fix for this; see [What I'd improve](#what-id-improve-with-more-time) for the structural fix this actually needs.

  As a side check, the same page, same field, run through a different vision model (Qwen3.8-27B via Groq — now wired in as a working second provider, see [Approach](#approach)) at `temperature: 0` came back correct 3 for 3. Small sample, different model/vendor, and it only tested the header fields rather than the full 40-item extraction — not a rigorous benchmark — but it's a useful signal that this is specifically about Gemini's reliability on this field, not an unavoidable property of vision-based extraction in general. Note the Groq provider only supports image input (PNG/JPEG), not PDF — Gemini remains the default and the only one of the two that handles this app's primary PDF workflow end-to-end.
...
- A secondary, non-Gemini cross-check (or fuzzy match) for `poNumber` extraction specifically — model config (`temperature`, `seed`) and prompt tuning were all tried and ruled out as a full fix (see [Known limitations](#known-limitations)), so this needs a structural solution, e.g. a deterministic regex/OCR pass on common "PO No" label patterns as a fallback or cross-check against Gemini's answer
